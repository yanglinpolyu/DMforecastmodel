...
#
#
load('dm_BOTH_[glm]_models_[HbA1c.cv]__.RData')
...
#        dev.off()
#
pdf.file.name = paste0('plot_TwoState_BOTH_model_[HbA1c.cv]_RR_[',temp.code,']_[sex.', temp.gender,']_[age.',temp.age.grp,']__v01.pdf')
...
for(ii in 1:length(HbA1c.cv.array)){#        ii = 3
  ...
  #
  temp.panel.lab = paste0('(',letters[temp.panel.index +ii],') ','CV of HbA1c = ',temp.HbA1c.cv)
