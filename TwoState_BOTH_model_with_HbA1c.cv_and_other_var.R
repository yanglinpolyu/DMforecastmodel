...
summary(curr.split.dm.lexis$HbA1c.cv)
...
#
kd.HbA1c.0.0 <- with( subset( curr.split.dm.lexis, lex.Cst== '0' & lex.Xst== '0'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.0.1 <- with( subset( curr.split.dm.lexis, lex.Cst== '0' & lex.Xst== '1'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.1.0 <- with( subset( curr.split.dm.lexis, lex.Cst== '1' & lex.Xst== '0'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
#
kd.HbA1c.1.1 <- with( subset( curr.split.dm.lexis, lex.Cst== '1' & lex.Xst== '1'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.1.2 <- with( subset( curr.split.dm.lexis, lex.Cst== '1' & lex.Xst== '2'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.2.1 <- with( subset( curr.split.dm.lexis, lex.Cst== '2' & lex.Xst== '1'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
#
kd.HbA1c.2.2 <- with( subset( curr.split.dm.lexis, lex.Cst== '2' & lex.Xst== '2'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.2.3 <- with( subset( curr.split.dm.lexis, lex.Cst== '2' & lex.Xst== '3'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
kd.HbA1c.3.2 <- with( subset( curr.split.dm.lexis, lex.Cst== '3' & lex.Xst== '2'), quantile(HbA1c.cv, probs=knots.pt, na.rm = T) )
#
...
mod.ad.0.0 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.0.0) + Ns(HbA1c.cv, knots = kd.HbA1c.0.0) + HbA1c.lvl 
...
mod.ad.0.1 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.0.1) + Ns(HbA1c.cv, knots = kd.HbA1c.0.1) + HbA1c.lvl 
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.1.0 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.1.0) + Ns(HbA1c.cv, knots = kd.HbA1c.1.0) + HbA1c.lvl #-1#
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.1.1 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.1.1) + Ns(HbA1c.cv, knots = kd.HbA1c.1.1) + HbA1c.lvl 
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.1.2 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.1.2) + Ns(HbA1c.cv, knots = kd.HbA1c.1.2) + HbA1c.lvl 
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.2.1 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.2.1) + Ns(HbA1c.cv, knots = kd.HbA1c.2.1) + HbA1c.lvl  #-1#
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.2.2 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.2.2) + Ns(HbA1c.cv, knots = kd.HbA1c.2.2) + HbA1c.lvl 
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.2.3 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.2.3) + Ns(HbA1c.cv, knots = kd.HbA1c.2.3) + HbA1c.lvl 
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
mod.ad.3.2 <- glm(
  (lex.Xst %in% curr.end.stage) ~ Ns(tfd, knots = kd.ad.3.2) + Ns(HbA1c.cv, knots = kd.HbA1c.3.2) + HbA1c.lvl  #-1#
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  ,
...
save(
  ...
  file = 'dm_BOTH_[glm]_models_[HbA1c.cv]__.RData'
)
...
#        dev.off()
c(1)
#            pdf('plot_TwoState_BOTH_model_HbA1c.cv_RR_[sex.F]_[ACEI.Y]_[ARB.Y]__v01.pdf', width = 11, height = 5)
#
HbA1c.cv.array = c(0.01,0.1,0.2,0.3,0.5,1,3)
#
par(las = 1, mar = c(3.75,4.25,1.5,1), mfrow = c(2,4), oma = c(0, 0, 2, 0))
#
for(ii in 1:length(HbA1c.cv.array)){#        ii = 3
  ...
  #
  temp.panel.lab = paste0('(',letters[ii],') ','SD of HbA1c = ',temp.HbA1c.cv)
  plot.one.rr(rr.list = temp.rr.list, panel.lab = temp.panel.lab)
  #
}
#
curr.var = na.omit(curr.split.dm.lexis$HbA1c.cv[which(
...
cx.ad.0.1 <- coxph(
  ...
  ~ Ns(HbA1c.cv, knots = kd.HbA1c.0.1) #+ Ns(tfd, knots = kd.ad.0.1)
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  , 
...
cx.ad.1.2 <- coxph(
  Surv(tfd, tfd + lex.dur, (lex.Xst %in% curr.end.stage)) 
  ~ Ns(HbA1c.cv, knots = kd.HbA1c.1.2) #+ Ns(tfd, knots = kd.ad.1.2)
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  , 
...
cx.ad.2.3 <- coxph(
  Surv(tfd, tfd + lex.dur, (lex.Xst %in% curr.end.stage)) 
  ~ Ns(HbA1c.cv, knots = kd.HbA1c.2.3) #+ Ns(tfd, knots = kd.ad.2.3)
  + sex + age + BMI + SBP + DBP# + smoke.status + Alcohol.status
  + Tot.Cholesterol + Creatinine + HDLC + LDLC + Triglycerides
  #+ is.Drug.ACEI + is.Drug.ARB
  , 
...
#save(cx.ad.0.1, cx.ad.1.2, cx.ad.2.3, file = 'dm_BOTH_[cox]_models_[HbA1c.cv]__.RData')
